  ...
  while ((c = getopt(argc, argv, "b:s:r:h:m:c:f:n:i:t:u:g:z:")) != -1) {
    switch (c) {
      ...
      case 'r':
        ...
        strcat(filename, "_EfCt2.csv");
        result_fpr = fopen(filename, "w");
        //strcpy(filename, optarg);
        //strcat(filename, "_EfCt_merge.csv");
        //result_fpr2 = fopen(filename, "w");
        break;
      case 'm':
...
void PrintStatRecord(TreeStat *p_record)
{
  ...
  printf("Rules: %d\n",p_record->No_Rules);
  printf("Depth: %d\n",p_record->Max_Depth);
  printf("Average Depth: %.2f\n",p_record->Total_Depth / float(p_record->Node_Count - p_record->NonLeaf_Node_Count));
  printf("Levels: %d\n",p_record->Max_Levels);
  printf("Cuts: %d\n",p_record->Max_Cuts);
...
void RecordTreeStats()
{
  ...
  p_record->Total_Depth = Total_Depth;
...
void create_tree(list <pc_rule*> p_classifier)
{
  ...
  while (!worklist.empty())
  {
    ...
    int max_children_rules = 0;
    for (list <node*>::iterator item = childlist.begin();
        item != childlist.end();++item)
    {
      curr_node->children.push_back(*item);
      //if ((*item)->classifier.size() > max_children_rules)
      //max_children_rules = (*item)->classifier.size();
    }

    //fprintf(result_fpr, "%f\n", max_children_rules/float(curr_node->classifier.size()));
