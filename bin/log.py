...
class Logger(object):
    ...
    @classmethod
    def start(cls, log_path, name=None, level='DEBUG'):
        ...
        fmt = ('[%(levelname)s] %(asctime)s, pid=%(process)d, '
               'src=%(pathname)s:%(lineno)d, %(message)s')
        datefmt = '%Y-%m-%d %H:%M:%S'
        cls._inst = logging.getLogger(name)
        cls._inst.setLevel(Logger._level_dict[level])

        handler = logging.handlers.RotatingFileHandler(log_path,
            maxBytes = 500 * (1<<20), backupCount = 8)
